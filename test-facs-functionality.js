...
console.log('-'.repeat(80));
const envContent = fs.readFileSync('./Maeple/.env', 'utf8');
...
for (const component of components) {
  const fullPath = path.join('./Maeple', component);
...
try {
  const packageJson = JSON.parse(fs.readFileSync('./Maeple/package.json', 'utf8'));
  ...
  if (hasBuildScript) {
    ...
    const distExists = fs.existsSync('./Maeple/dist');
