    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-gradient-to-r from-purple-600 to-pink-500 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <p className="text-purple-100 text-base max-w-xl leading-relaxed mb-6">
          Process your emotions visually. Use generative art to externalize feelings, visualize safe spaces, 
          or celebrate your strengths.
        </p>
        ...
        {recentEntry && (
            <div className="bg-white/10 border border-white/20 rounded-xl p-4 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap gap-2">
                    {/* Mood Prompt */}
                    <button 
                        onClick={() => useSmartPrompt(`An abstract artistic representation of feeling ${recentEntry.moodLabel}, expressive colors`)}
                    ...
                    {recentEntry.neuroMetrics.sensoryLoad > 6 && (
                        <button 
                            onClick={() => useSmartPrompt("A calm, quiet sanctuary with soft lighting and nature elements, hyper-realistic")}
                            className="px-3 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-xs text-white transition-colors text-left flex items-center gap-2 border border-white/10"
                        >
                    ...
                    {recentEntry.strengths.length > 0 && (
                        <button 
                            onClick={() => useSmartPrompt(`A vibrant, energetic metaphorical illustration of the character strength: ${recentEntry.strengths[0]}`)}
                            className="px-3 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-xs text-white transition-colors text-left flex items-center gap-2 border border-white/10"
                        >
          ...
          <div className="flex gap-4">
            <div className="flex-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
               <input 
                  ...
                  id="img-upload" 
               />
               <label htmlFor="img-upload" className="flex items-center justify-center gap-2 h-10 cursor-pointer hover:bg-slate-50 rounded-lg transition-colors text-sm text-slate-600 font-medium">
                  <ImagePlus size={16} />
                  {upload ? "Change Reference Image" : "Upload Reference (Optional)"}
               </label>
            </div>
          </div>
          ...
          <div className="relative group">
             ...
             <div className="relative bg-white rounded-xl">
                <textarea 
                    ...
                    placeholder={upload ? "How should we edit this image? (e.g. 'Add a healthy salad next to it')" : "Describe what you want to visualize... (e.g. 'My mind as a garden recovering from a storm')"}
                    className="w-full bg-transparent rounded-xl p-4 text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-0 h-32 resize-none text-lg"
                />
      ...
      <div className="grid md:grid-cols-2 gap-6">
          {upload && (
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <span className="text-xs font-bold text-slate-400 uppercase mb-2 block tracking-wider">Original Reference</span>
                <div className="rounded-xl overflow-hidden bg-slate-50 border border-slate-100 aspect-square">
                    <img src={upload} alt="Original" className="w-full h-full object-cover" />
                </div>
            </div>
          )}
          {generatedImage && (
             <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm md:col-span-2">
                ...
                <div className="mt-4 p-4 bg-purple-50 rounded-xl border border-purple-100">
                    <h4 className="text-sm font-bold text-purple-900 mb-1">Reflection</h4>
                    <p className="text-sm text-purple-700">
                        How does looking at this image make you feel? Consider saving it to look at when you need a reminder of this state.
                    </p>
                </div>
            </div>
          )}
