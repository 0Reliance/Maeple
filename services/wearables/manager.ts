...
class WearableManager {
  ...
  private initializeAdapters() {
    ...
    console.log(
      `[Wearables] Apple Health: ${
        appleHealthAdapter.isNativeMode()
          ? "Native HealthKit"
          : "Web simulation"
      }`
    );
    ...
    // Whoop adapter
    const whoopAdapter = new WhoopAdapter();
    this.adapters.set("WHOOP", whoopAdapter);
    console.log("[Wearables] Whoop: Initialized (Mock/Dev mode)");

    // Log available providers
    console.log(
      `[Wearables] Available providers: ${Array.from(this.adapters.keys()).join(
        ", "
      )}`
    );
  }
  ...
  getProviderInfo(provider: ProviderType): {
    ...
    const providerInfo: Record<
      ProviderType,
      { name: string; description: string; requiresNative: boolean }
    > = {
      OURA: {
        name: "Oura Ring",
  ...
  private isAdapterConfigured(adapter: WearableAdapter): boolean {
    if (
      ...
      typeof (adapter as any).isConfigured === "function"
    ) {
  ...
  async syncAllProviders(daysBack = 7): Promise<WearableDataPoint[]> {
    ...
    for (const [provider, config] of Object.entries(configs)) {
      if (config.isConnected) {
        try {
          const points = await this.syncRecentData(
