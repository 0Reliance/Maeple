...
interface SyncStoreActions {
  ...
  syncAll: () => Promise<{ success: boolean; pushed: number; pulled: number; error?: string }>;
    ...
    (set, get) => ({
      ...
      initialize: async () => {
        ...
        if (!isSupabaseConfigured()) {
          set({ isInitialized: true, status: 'offline' }, false, 'initialize:noSupabase');
          return;
        }
