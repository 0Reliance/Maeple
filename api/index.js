...
const JWT_SECRET = process.env.JWT_SECRET || 'maeple-beta-secret-2025-change-in-production';
...
app.use((req, res, next) => {
  ...
  res.json = function(data) {
    ...
    if (performanceMetrics.responseTimes.length > 100) {
      performanceMetrics.responseTimes = performanceMetrics.responseTimes.slice(-50); // Keep last 50
    }
    ...
    console.log(`[${new Date().toISOString()}] ${req.method} ${req.path} - Response sent (${duration}ms)`);
...
app.get('/api/health', async (req, res) => {
  ...
  try {
    ...
    console.log(`[Health] System health check completed in ${Date.now() - startTime}ms`);
    res.json(healthData);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        await client.query(
          ...
          [
            entryId, req.user.id, entry.timestamp || new Date(), entry.rawText || '', entry.mood || null,
            entry.moodLabel || '', JSON.stringify(entry.medications || []), JSON.stringify(entry.symptoms || []),
            entry.tags || [], entry.activityTypes || [], entry.strengths || [],
            JSON.stringify(entry.neuroMetrics || {}), JSON.stringify(entry.sleep || {}), entry.notes || '',
            JSON.stringify(entry.aiStrategies || []), entry.aiReasoning || '', entry.timestamp || new Date()
          ]
        );
        synced++;
      }
    ...
    return res.status(404).json({
      ...
      availableEndpoints: {
        health: [
          ...
          { method: 'GET', path: '/api/health/simple', description: 'Simple health check' },
          { method: 'GET', path: '/api/metrics', description: 'Performance metrics' }
        ],
        auth: [
          { method: 'POST', path: '/api/auth/signup', description: 'Create new account' },
          { method: 'POST', path: '/api/auth/signin', description: 'Sign in existing user' },
          { method: 'GET', path: '/api/auth/me', description: 'Get current user' },
          { method: 'POST', path: '/api/auth/signout', description: 'Sign out user' },
          { method: 'POST', path: '/api/auth/change-password', description: 'Change password' },
          { method: 'DELETE', path: '/api/auth/account', description: 'Delete account' }
        ],
        entries: [
          { method: 'GET', path: '/api/entries', description: 'List all entries' },
          { method: 'POST', path: '/api/entries', description: 'Create new entry' },
          { method: 'PUT', path: '/api/entries/:id', description: 'Update existing entry' },
          { method: 'DELETE', path: '/api/entries/:id', description: 'Delete entry' },
          { method: 'POST', path: '/api/entries/sync', description: 'Bulk sync entries' }
        ],
        settings: [
